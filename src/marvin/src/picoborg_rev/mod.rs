...
impl PicoBorgRev {
    ...
    pub fn set_motor_1(&mut self, power: f32) -> Result<(), LinuxI2CError> {
        if power < 0.0 {
            self.device
                .smbus_write_byte_data(COMMAND_SET_A_REV, (PWM_MAX * -power) as u8)
        } else {
            self.device
                .smbus_write_byte_data(COMMAND_SET_A_FWD, (PWM_MAX * power) as u8)
        }
    }
    ...
    pub fn set_motor_2(&mut self, power: f32) -> Result<(), LinuxI2CError> {
        if power < 0.0 {
            self.device
                .smbus_write_byte_data(COMMAND_SET_B_REV, (PWM_MAX * -power) as u8)
        } else {
            self.device
                .smbus_write_byte_data(COMMAND_SET_B_FWD, (PWM_MAX * power) as u8)
        }
    }
    ...
    pub fn set_motors(&mut self, power: f32) -> Result<(), LinuxI2CError> {
        if power < 0.0 {
            self.device
                .smbus_write_byte_data(COMMAND_SET_ALL_REV, (PWM_MAX * -power) as u8)
        } else {
            self.device
                .smbus_write_byte_data(COMMAND_SET_ALL_FWD, (PWM_MAX * power) as u8)
        }
    }

    /// Sets all motors to stopped, useful when ending a program
    pub fn motors_off(&mut self) -> Result<(), LinuxI2CError> {
        self.device.smbus_write_byte_data(COMMAND_ALL_OFF, 0)
    }
    ...
    pub fn encoder_move_motor_1(&mut self, counts: i16) -> Result<(), LinuxI2CError> {
        if counts < 0 {
            self.device
                .smbus_write_block_data(COMMAND_MOVE_A_REV, &u16_to_vu8(-counts as u16))
        } else {
            self.device
                .smbus_write_block_data(COMMAND_MOVE_A_FWD, &u16_to_vu8(counts as u16))
        }
    }
    ...
    pub fn encoder_move_motor_2(&mut self, counts: i16) -> Result<(), LinuxI2CError> {
        if counts < 0 {
            self.device
                .smbus_write_block_data(COMMAND_MOVE_B_REV, &u16_to_vu8(-counts as u16))
        } else {
            self.device
                .smbus_write_block_data(COMMAND_MOVE_B_FWD, &u16_to_vu8(counts as u16))
        }
    }
    ...
    pub fn encoder_move_motors(&mut self, counts: i16) -> Result<(), LinuxI2CError> {
        if counts < 0 {
            self.device
                .smbus_write_block_data(COMMAND_MOVE_ALL_REV, &u16_to_vu8(-counts as u16))
        } else {
            self.device
                .smbus_write_block_data(COMMAND_MOVE_ALL_FWD, &u16_to_vu8(counts as u16))
        }
    }
    ...
    pub fn set_encoder_speed(&mut self, power: f32) -> Result<(), LinuxI2CError> {
        self.device
            .smbus_write_byte_data(COMMAND_SET_ENC_SPEED, (PWM_MAX * power) as u8)
    }
